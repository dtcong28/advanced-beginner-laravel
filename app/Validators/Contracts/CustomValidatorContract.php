...
class CustomValidatorContract extends BaseValidatorContract
{
    ...
    public function validateKatakanaWithSpace($attribute, $value, $parameters)
    {
        $result = preg_match('/^([　 ゠ァアィイゥウェエォオカガキギクグケゲコゴサザシジスズセゼソゾタダチヂッツヅテデトドナニヌネノハバパヒビピフブプヘベペホボポマミムメモャヤュユョヨラリルレロヮワヰヱヲンヴヵヶヷヸヹヺ・ーヽヾヿ]+)$/u', $value, $matches);
    ...
    public function validateJapanese($attribute, $value, $parameters)
    {
        return $this->isJapanese($value);
    }

    public function validateFullWidth($attribute, $value, $parameters)
    {
        $result = preg_match('/^[ぁ-んァ-ン一-龥０-９！＂＃＄％＆＇（）＊＋，－．／：；＜＝＞？＠［＼］＾＿｀｛｜｝～]+$/', $value, $matches);

        return $result && !preg_match('/[ｧ-ﾝﾞﾟ]/u', $value, $matches) ? true : false;
    }
