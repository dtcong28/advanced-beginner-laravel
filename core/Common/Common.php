...
if (!function_exists('getGmoConfig')) {
    /**
     * @param $key
     * @param $default
     * @return Repository|Application|mixed
     */
    function getGmoConfig($key, $default = null)
    {
        return config('gmo.' . $key, $default);
    }
}

if (!function_exists('getOrderIdForBatch')) {
    /**
     * @param $memberId
     * @return string
     */
    function getOrderIdForBatch($memberId)
    {
        if (empty($memberId)) {
            return "";
        }
        $orderID = \Carbon\Carbon::now()->addMonth(1)->format('Ymd');
        for ($i = 0; $i < 7 - mb_strlen($memberId); $i++) {
            $orderID .= '0';
        }
        $orderID .= $memberId;
...
if (!function_exists('isEmptyCsv')) {
    function isEmptyCsv($value): bool
    {
        if (!isset($value)) {
            return true;
        }
